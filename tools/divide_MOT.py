...
def divide_dataset(seqs, root='', val=0.5, gap=30, gstart=0, tag='st.i'):
    gt_dir = os.path.join(root, 'train/%s/gt/gt.txt')
    dt_dir = os.path.join(root, 'train/%s/det/det.txt')
    img_dir = os.path.join(root, 'train/%s/img1/')
    trains = []
    train_f = 0
    train_b = 0
    vals = []
    ...
    for seq in seqs:
        ...
        if train_n:
            ...
            with open(os.path.join(gt_dir % seq_t), 'w') as fd:
                new_trkset.dump(fd, formatter='fr.i, id.i, x1, y1, w, h, %s, la.i, cf'%tag)
            with open(os.path.join(dt_dir % seq_t), 'w') as fd:
                new_trkset2.dump(fd, formatter='fr.i, id.i, x1, y1, w, h, cf, -1, -1, -1')
            iniparser = configparser.ConfigParser()
            iniparser.read(os.path.join(root, 'train', seq, 'seqinfo.ini'))
            iniparser['Sequence']['SeqLength'] = str(val_n)
            with open(os.path.join(root, 'train', seq_t, 'seqinfo.ini'), 'w') as fd:
                iniparser.write(fd)
            trains.append(seq_t)
        if val_n:
            seq_t = seq + '-v'
