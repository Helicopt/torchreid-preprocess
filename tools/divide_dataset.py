...
HIE20_root = '/mnt/lustre/share/fengweitao/HIE20'
MOT16_train = ['MOT16-%02d'%i for i in [2, 4, 5, 9, 10, 11, 13]]
MOT16_root = '/mnt/lustre/share/fengweitao/MOT16'

def divide_dataset(seqs, root='', val=0.3, gap=30, gstart=0, tag='st.i'):
    gt_dir = os.path.join(root, 'gts', 'train')
    dt_dir = os.path.join(root, 'dts', 'train')
    img_dir = os.path.join(root, 'imgs', 'train')
    trains = []
    train_f = 0
    train_b = 0
    vals = []
        ...
        if train_n:
            ...
            for i in range(train_n):
                ...
                for d in gt[i + gstart]:
                    ...
                    train_b += 1
                train_f += 1
            with open(os.path.join(gt_dir, seq_t + '.txt'), 'w') as fd:
                new_trkset.dump(fd, formatter='fr.i, id.i, x1, y1, w, h, %s, -1, -1, -1'%tag)
            with open(os.path.join(dt_dir, seq_t + '.txt'), 'w') as fd:
...
if __name__=='__main__':
    ...
    divide_dataset(MOT16_train, root=MOT16_root, val=0.3, gap=30, gstart=1, tag='st.i')
